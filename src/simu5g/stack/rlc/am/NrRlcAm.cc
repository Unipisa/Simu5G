...
void NrRlcAm::handleUpperMessage(cPacket *pktAux)
{
    ...
    NrAmTxQueue *txbuf = getNrTxBuffer(ctrlInfoToUeId(lteInfo.get()), lteInfo->getLcid());

    // Extract sequence number from PDCP header (main 3cbd15bc moved it there)
    ...
    EV << NOW << " NrRlcAm::handleUpperMessage sending " << pkt
       << " to AM TX Queue sn=" << sequenceNumber << endl;
    txbuf->enque(pkt);
}
void NrRlcAm::handleLowerMessage(cPacket *pktAux)
{
    ...
    if (inet::dynamicPtrCast<const LteMacSduRequest>(chunk) != nullptr) {
        ...
        NrAmRxQueue *rxbuf = getNrRxBuffer(ctrlInfoToUeId(lteInfo.get()), lteInfo->getLcid());
        drop(pkt);
