...
void NrAmRxQueue::enque(Packet *pkt)
{
    ...
    // Extract FlowControlInfo + IpFlowInd on first PDU (swap src/dst for status reports)
    if (flowControlInfo_ == nullptr) {
        ...
        auto origIpFlow = pkt->findTag<IpFlowInd>();
        if (origIpFlow) {
            ipFlowInd_ = origIpFlow->dup();
            ipFlowInd_->setSrcAddr(origIpFlow->getDstAddr());
            ipFlowInd_->setDstAddr(origIpFlow->getSrcAddr());
            ipFlowInd_->setTypeOfService(origIpFlow->getTypeOfService());
        }
    }
...
void NrAmRxQueue::passUp(int seqNum)
{
    ...
    // Derive flow info from stored flowControlInfo_ (which has src/dst reversed for status reports)
    // Original sender = flowControlInfo_->getDestId(), original receiver = flowControlInfo_->getSourceId()
    Direction dir = (flowControlInfo_->getDirection() == DL) ? UL : DL;
    MacNodeId srcId = flowControlInfo_->getDestId();
    MacNodeId dstId = flowControlInfo_->getSourceId();
    cModule *nodeb = nullptr;
    cModule *ue = nullptr;
    double delay = (NOW - sdu->getCreationTime()).dbl();

    if (dir == DL) {
        nodeb = binder_->getRlcByNodeId(srcId, AM);
        ue = binder_->getRlcByNodeId(dstId, AM);
    }
    else {
        nodeb = binder_->getRlcByNodeId(dstId, AM);
        ue = binder_->getRlcByNodeId(srcId, AM);
    }
    ...
    // Restore FlowControlInfo for PDCP (cleared by RLC pushSdu) and remove PdcpTrackingTag
    auto ci = sdu->addTagIfAbsent<FlowControlInfo>();
    ci->setSourceId(srcId);
    ci->setDestId(dstId);
    ci->setDirection(dir);
    ci->setLcid(flowControlInfo_->getLcid());
    ci->setRlcType(AM);
...
{
    remoteEntity_ = binder_->getRlcByNodeId(id, AM);
}
...
void NrAmRxQueue::sendStatusReport()
{
    ...
    *pktPdu->addTagIfAbsent<FlowControlInfo>() = *flowControlInfo_;
    if (ipFlowInd_)
        *pktPdu->addTagIfAbsent<IpFlowInd>() = *ipFlowInd_;
