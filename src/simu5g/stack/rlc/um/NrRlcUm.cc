...
NrUmTxEntity *NrRlcUm::getNrTxBuffer(inet::Ptr<FlowControlInfo> lteInfo)
{
    ...
    LogicalCid lcid = 0;
    if (lteInfo != nullptr) {
        nodeId = ctrlInfoToUeId(lteInfo.get());
...
void NrRlcUm::handleUpperMessage(cPacket *pktAux)
{
    ...
    // Extract sequence number from PDCP header (main 3cbd15bc)
    auto pdcpHeader = pkt->peekAtFront<LtePdcpHeader>();
    unsigned int sequenceNumber = pdcpHeader->getSequenceNumber();

    // Tag with PDCP tracking info (replaces topic's LteRlcSdu chunk)
    ...
    if (txbuf->isHoldingDownstreamInPackets()) {
        ...
        EV << "NrRlcUm::handleUpperMessage - Enqueue packet into the Holding Buffer\n";
        txbuf->enqueHoldingPackets(pkt);
    }
    else {
        if (txbuf->enque(pkt)) {
            EV << "NrRlcUm::handleUpperMessage - Enqueue packet into the Tx Buffer\n";
            // notify the MAC layer that the queue contains new data, via tag (replaces LteRlcPduNewData chunk)
            auto pktDup = check_and_cast<inet::Packet*>(pkt)->dup();
            pktDup->addTag<LteRlcNewDataTag>();

            EV << "NrRlcUm::handleUpperMessage - Sending new-data notification to port UM_Sap_down$o\n";
