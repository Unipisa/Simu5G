...
//    auto pkt = check_and_cast<inet::Packet *> (pktAux);
    ...
    int64_t  sduSize = pdcpPkt->getByteLength();
...
////
    ...
    EV << "PacketFlowManagerEnb::insertPdcpSdu - DL PDPC sdu bits: " << sduDataVolume_[nodeId].dlBits << " sent to node: " << nodeId << endl;
...
void PacketFlowManagerEnb::receivedPdcpSdu(inet::Packet* pdcpPkt)
{
    ...
    EV << "PacketFlowManagerEnb::insertPdcpSdu - UL PDPC sdu bits: " << sduDataVolume_[nodeId].ulBits << " received from node: " << nodeId << endl;

    }

void PacketFlowManagerEnb::insertRlcPdu(LogicalCid lcid, inet::Ptr<LteRlcUmDataPdu> rlcPdu, RlcBurstStatus status) {
     EV << "PacketFlowManagerEnb::insertRlcPdu - Logical Cid: " << lcid << endl;
...
//                     totalPdcpSduSize += pdcpPduLength();
//                 else if(i == 0 && (fi == 0 || fi == 1)) // remove pdcp header, and adjust the size of the pdcp sdu
//                     totalPdcpSduSize += (pdcpPduLength - (lteInfo->getRlcType() == UM ? PDCP_HEADER_UM : PDCP_HEADER_AM) - headerCompressedSize_ + pit->second.pdcpSduSize);
...
void PacketFlowManagerEnb::insertMacPdu(LogicalCid lcid, inet::Ptr<const LteMacPdu> macPdu)
{
    EV << "PacketFlowManagerEnb::insertMacPdu" << endl;
    ...
    for(int i = 0; i < len; ++i)
    {
        ...
        unsigned int rlcSno = macPdu->getSdu(i).peekAtFront<LteRlcUmDataPdu>()->getPduSequenceNumber();
...
void PacketFlowManagerEnb::macPduArrived(LogicalCid lcid, unsigned int macPduId)
{
    ...
    if (cit == connectionMap_.end())
    {
        ...
        throw cRuntimeError("PacketFlowManagerEnb::macPduArrived - Logical CID %d not present. It must be initilized before", lcid);
        return;
    }
