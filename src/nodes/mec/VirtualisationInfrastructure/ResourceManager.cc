...
bool ResourceManager::registerMecApp(int ueAppID, int reqRam, int reqDisk, double reqCpu)
{
    ...
    if(availableResources)
    {
        ...
        resourceMap[ueAppID].disk = reqDisk;
        resourceMap[ueAppID].cpu = reqCpu;
        EV << "ResourceManager::handleMEAppResources - resources ALLOCATED for independent MecApp with module id " << ueAppID  << endl;
        EV << "ResourceManager::handleMEAppResources - ram: " << resourceMap[ueAppID].ram <<" disk: "<< resourceMap[ueAppID].disk <<" cpu: "<< resourceMap[ueAppID].cpu << endl;
        allocateResources(reqRam, reqDisk, reqCpu);
        return true;
    }
    else{
        EV << "ResourceManager::handleMEAppResources - resources NOT AVAILABLE for independent MecApp with module id " << ueAppID  << endl;
        return false;
    }
}

bool ResourceManager::deRegisterMecApp(int ueAppID)
{
    if(!resourceMap.empty() || resourceMap.find(ueAppID) != resourceMap.end())
    {
        EV << "ResourceManager::handleMEAppResources - resources DEALLOCATED for independent MecApp with module id " << ueAppID  << endl;
        EV << "ResourceManager::handleMEAppResources - ram: " << resourceMap[ueAppID].ram <<" disk: "<< resourceMap[ueAppID].disk <<" cpu: "<< resourceMap[ueAppID].cpu << endl;
        deallocateResources(resourceMap[ueAppID].ram, resourceMap[ueAppID].disk, resourceMap[ueAppID].cpu);
        //erasing map entry
        resourceMap.erase(ueAppID);
    ...
    {
        EV << "ResourceManager::handleMEAppResources - NO ALLOCATION FOUND for independent MecApp with module id " << ueAppID  << endl;
        return false;
    }
}
...
double ResourceManager::calculateProcessingTime(int ueAppID, int numOfInstructions)
{
    ...
    auto ueApp = resourceMap.find(ueAppID);
