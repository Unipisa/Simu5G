...
MAKEMAKE_OPTIONS := $(MAKEMAKE_OPTIONS) $(shell $(FEATURETOOL) options -f -l) -KINET_PROJ=$(INET_ROOT) -KGEOLIB=../../geographiclib -DINET_IMPORT -I. -I$$\(INET_PROJ\)/src -I$$\(GEOLIB\)/src -L$$\(INET_PROJ\)/src -L$$\(GEOLIB\)/BUILD/src -lINET$$\(D\) -lGeographicLib
