...
sub startTraffic
{
    ...
    print SOCKET "backgroundUes-ca=1-u=$par_numerology-rbs=$par_rbs-sndInt=$par_sndInt-bkUEs=$par_ues-dynamicPhy=false-#$par_run\n";
            ...
            foreach my $extCells (@extCells)
            {
                foreach my $sndInt (@sndInterval)
                {
                    foreach my $run (@runs)
                    {
                        ...
                        print $configOut "include trafficGeneratorConfigs/config-u=${numerology}-rbs=${rbs}-numBgCells=0-numBkUEs=${ues}-dist=50m-repetition=0.ini";
                        close $configOut;
